...
export class TrainSensors {
  ...
  async updateSensors(lineStatus: TrainStatusReturn[]) {
    Logger.info('TrainSensors', 'Updating sensors', { lineStatus })
    await Promise.all(
      lineStatus.map(line => {
        const codeInWords = numberToWords(line.codigo)
        if (!codeInWords) return
        const sensor = new Sensor(
          `sensor.sp_train_${codeInWords}`,
          `sensor.sp_train_${codeInWords}`,
          {
  ...
  async getTrainLineData(lineCodeInWords: string) {
    Logger.info('TrainSensors', 'Getting train line data', { lineCodeInWords })
    const sensor = new Sensor(
      `sensor.sp_train_${lineCodeInWords}`,
      `sensor.sp_train_${lineCodeInWords}`,
      {
    ...
    Logger.info('TrainSensors', 'Train line data', { response })
    return response
  }
}
